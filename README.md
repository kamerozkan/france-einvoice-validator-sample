...
The evaluated fixtures are committed under [`fixtures/`](fixtures/). Their SHA-256 values match the real output rows. The third input intentionally uses a non-HTTPS URL to demonstrate the source-safety boundary.
...
Latest hosted build `0.0.7` was verified `SUCCEEDED` on 2026-07-30. The sample rows remain attributed to their original build `0.0.4`; they are not relabeled as current-build output.
